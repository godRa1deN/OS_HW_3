...
void receive_client_string(int client_socket, char *buffer, size_t buffer_size) {
...
int main(int argc, char *argv[]) {
    ...
    if (listen(server_socket, num_clients + 1) < 0) {
        perror("Ошибка в прослушивании подключений на сервере");
        exit(EXIT_FAILURE);
    }

    printf("Сервер запущен. Ожидание подключения state программы и %d клиентов...\n", num_clients);

    // Прием подключения от state программы
    int state_program_socket;
    if ((state_program_socket = accept(server_socket, (struct sockaddr *)&client_address, (socklen_t *)&addrlen)) < 0) {
        perror("Ошибка в принятии подключения от state программы");
        exit(EXIT_FAILURE);
    }
    printf("State программа подключена\n");

    // Отправка сообщения state программе
    char server_start_message[1024];
    sprintf(server_start_message, "Сервер запущен. Ожидание подключения state программы и %d клиентов...\n", num_clients);
    send_string(state_program_socket, server_start_message);

    // Прием подключений от клиентов
    int encoded_array[length];
    char client_receive_string[1024];
    for (i = 0; i < num_clients; i++) {
        ...
        sprintf(server_start_message, "Клиент %d подключен\n", i + 1);
        send_string(state_program_socket, server_start_message);
        ...
        //Получение состояния от клиента
        receive_client_string(client_sockets[i], client_receive_string, sizeof(client_receive_string));

        //Отрпавка состояние клиента в state программу 
        send_string(state_program_socket, client_receive_string);
        ...
        sprintf(client_receive_string, "Сервер получил закодированный массив от клиента %d\n", i + 1);
        send_string(state_program_socket, client_receive_string);
    ...
    // Закрытие соединений с клиентами и state программой
    for (i = 0; i < num_clients; i++) {
        close(client_sockets[i]);
    }
    close(state_program_socket);
    close(server_socket);
